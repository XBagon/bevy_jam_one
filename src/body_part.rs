...
impl BodyPart {
    pub fn setup(mut commands: Commands, ase_file_map: Res<AseFileMap>) {
        ...
        let target_assets = ase_file_map.get(Path::new("sprites/body_parts.aseprite")).unwrap();
    ...
    pub fn win_check(mut ev_phase: EventReader<game::Phase>, query_pipeline: Res<QueryPipeline>, collider_query: QueryPipelineColliderComponentsQuery, q_body_part: Query<&RigidBodyActivationComponent, With<BodyPart>>, mut ev_won: EventWriter<game::Won>) {
        for ev in ev_phase.iter() {
            match ev {
                game::Phase::TransMain => {
                    let collider_set = QueryPipelineColliderComponentsSet(&collider_query);

                    let ray = Ray::new(Point::new(-HALF_WIDTH / PHYSICS_SCALE, 0.).into(), Vector2::new(1.0, 0.0).into());
